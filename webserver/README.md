...
│   ├── domain/          # 领域模型模块
│   │   ├── mod.rs       # 领域模块声明
│   │   ├── subscriber_name.rs    # 订阅者姓名验证
│   │   ├── subscriber_email.rs   # 订阅者邮箱验证
│   │   └── new_subscriber.rs     # 新订阅者模型
│   └── routes/          # 路由模块
│       ├── mod.rs       # 路由模块声明
│       ├── greet.rs     # 问候功能
│       ├── health.rs    # 健康检查
...
- `GET /health` - 健康检查端点
- `POST /subscribe` - 用户订阅端点（需要验证姓名和邮箱格式）
...
### 订阅者姓名验证 (SubscriberName)
- **长度限制**: 最多 256 个字符（使用 Unicode 分段计算）
- **禁止字符**: `/`, `(`, `)`, `"`, `<`, `>`, `\`, `{`, `}`
- **空值检查**: 拒绝空字符串和仅包含空白字符的输入

### 订阅者邮箱验证 (SubscriberEmail)
- **格式验证**: 使用 `validator::validate_email` 进行 RFC 5322 标准验证
- **自动转换**: 邮箱地址自动转换为小写
- **类型安全**: 使用新类型模式确保只有有效邮箱才能创建 `SubscriberEmail` 实例
...
// 创建有效的邮箱
let email = SubscriberEmail::parse("user@example.com".to_string())
    .expect("有效的邮箱");

// 无效输入会返回错误
let invalid_name = SubscriberName::parse("".to_string());
...
- `src/main.rs` - 应用程序入口点
- `src/domain/` - 领域模型和业务逻辑
  - `subscriber_name.rs` - 订阅者姓名验证逻辑
  - `subscriber_email.rs` - 订阅者邮箱验证逻辑
  - `new_subscriber.rs` - 新订阅者聚合模型
- `tests/` - 测试文件目录

### 添加新功能

1. 在 `src/lib.rs` 中添加新的处理器函数
2. 在 `startup.rs` 的 `run()` 函数中注册新路由
3. 如需新的领域模型，在 `src/domain/` 中创建
4. 添加相应的单元测试（使用 Claim 进行断言）
...
- **v1.5.0** - 添加数据验证功能
  - 实现 `SubscriberName` 和 `SubscriberEmail` 验证
  - 集成 Validator 库进行邮箱格式验证
  - 添加 Claim 测试框架
  - 完善错误处理和类型安全
